...
(function () {
	...
	gpii.matchMakerFramework.tryGetPreferredMatchMaker = function (matchMakerPayload) {
		// Constants
		var USER_DEFINED_MATCHMAKER_PREF = "http://registry.gpii.org/common/matchMakerType";
		// Rejection tests
		if (!("preferences" in matchMakerPayload)) return { "found": false };
		if (!("contexts" in matchMakerPayload.preferences)) return { "found": false };
		if (!("activeContexts" in matchMakerPayload)) return { "found": false };
		...
		fluid.each(activeContexts, function (activeContext) {
			if ((activeContext in contexts) && ("preferences" in contexts[activeContext])) {
				...
				var curPriority = 0;
				if ("priority" in contexts[activeContext]) curPriority = contexts[activeContext].priority;
		...
		if (typeof foundMatchMaker != 'undefined')
			return { "found": true, "matchMaker": foundMatchMaker };
