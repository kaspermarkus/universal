    ...
    */
    ...
    gpii.matchMakerFramework.utils.buildReturnPayload = function (fullModel, disposed) {
        ...
        fluid.each(disposed, function (solArray, contextId) {
            ...
            if (fullModel.outputMetadata && fullModel.outputMetadata[contextId]) {
                togo.inferredConfiguration[contextId].metadata = fullModel.outputMetadata[contextId];
            }
    ...
    gpii.matchMakerFramework.utils.addPCPPopulation = function (fullModel) {
        var metadata = fullModel.outputMetadata || {};
        //search through preferences for a metaData block related to PCP content:
        fluid.each(fullModel.preferences.contexts, function (content, contextId) {
            var OS = fullModel.deviceReporter.OS.id;
            if (OS === "darwin") {
                if (!metadata[contextId]) {
                    metadata[contextId] = [];
                }
                metadata[contextId] = (metadata[contextId]).concat([
                    {
                        "type": "PCPPopulation",
                        "settings": {
                            "org.nvda-project": {
                                "settings": {
                                    "http://registry.gpii.net/applications/org.gnome.desktop.a11y.magnifier/mag-factor": 0.1
        ...
        fullModel.outputMetadata = metadata;
    };

    gpii.matchMakerFramework.utils.addUserMessages = function (fullModel) {
        var metadata = fullModel.outputMetadata || {};
        //search through preferences for a metaData block related to PCP content:
        fluid.each(fullModel.preferences.contexts, function (content, contextId) {
            if (!metadata[contextId]) {
            ...
            metadata[contextId] = (metadata[contextId]).concat([{
                ...
                message: {
                    ...
                    en: "Straight from the matchmaker",
