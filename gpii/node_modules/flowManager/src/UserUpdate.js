...
(function () {
    ...
    gpii.request.flowManager.userUpdateHandle = function (event, fluidRequest, transformer, lifecycleManager, that) {
        console.log("USER UPDATE HANDLE CALLED");
        that.withSession(function (session) {
            var updateData = gpii.request.flowManager.formatToConfiguration(fluidRequest.data);
    ...
    //     }
    // }
    gpii.request.flowManager.formatToConfiguration = function (data) {
        var togo = {};
        fluid.each(data, function (app, appId) {
            if (appId === "common") return; //ignore anything in common as we dont know how to translate into app specific settings
            togo[appId] = {};
            togo[appId].settings = {};
